...
class App extends Component {
  ...
  filterByEmploymentType = () => {
    ...
    if (employmentTypeFT === true && employmentTypePT === true) {
      ...
      this.setState({
        filteredJobs: populateFilteredJobs.filter(
          job => job.hours === "Full-time"
        )
      });
    } else if (employmentTypePT === true) {
      this.setState({
        filteredJobs: populateFilteredJobs.filter(job =>
          job.hours.includes("Part-time")
        )
      });
    }

    populateFilteredJobs = this.state.filteredJobs.filter(job =>
      job.title.toLowerCase().includes(this.props.searchField.toLowerCase())
    );
    ...
    return isPending ? (
      ...
      <Router>
        <div className="App">
          ...
          <Route
            path="/organizations"
            render={() => <Organizations organizationData={organizationData} />}
          />
